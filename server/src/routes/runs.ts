...
router.post('/upload', requireApiToken, (req: Request, res: Response) => {
  const { client_slug, run_date, audit_seo, audit_geo, audit_aeo, fixes, pages_audit, report_md } = req.body
  ...
  const run = db.prepare(`
    INSERT INTO runs (client_id, run_date, audit_seo, audit_geo, audit_aeo, fixes, pages_audit, report_md)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
router.get('/', requireAuth(), (req: Request, res: Response) => {
  ...
  if (role === 'superadmin') {
    runs = db.prepare(`
      SELECT r.id, r.run_date, r.status, r.created_at, r.published_at, c.slug, c.name
...
router.get('/:id', requireAuth(), (req: Request, res: Response) => {
  ...
  // Parse JSON fields
  const jsonFields = ['audit_seo', 'audit_geo', 'audit_aeo', 'fixes', 'pages_audit']
